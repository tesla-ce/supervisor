...
def deploy_tesla_workers_view(request):
    ...
    context = {
        'options': {
            ...
            'deployment_specialized_workers': client.tesla.get("DEPLOYMENT_SPECIALIZED_WORKERS")
        }
    }
    if request.method == 'GET':
        if request.GET.get('deployment_specialized_workers') is not None:
            val = True if str(request.GET.get('deployment_specialized_workers')) is '1' else False
            client.tesla.get_config().set("DEPLOYMENT_SPECIALIZED_WORKERS", val)
    ...
    context = {
        'options': {
            ...
            'data_path': client.tesla.get("DEPLOYMENT_DATA_PATH"),
            'steps': ['configure_vault', 'db_migration', 'create_superuser', 'collect_statics', 'fixtures'],
            'deployment_specialized_workers': client.tesla.get_config().get("DEPLOYMENT_SPECIALIZED_WORKERS")
        }
    }
    # client.tesla.persist_configuration()
    ...
    if request.method == 'POST':
        ...
        client.tesla.get_config().set('DEPLOYMENT_STATUS', 11)
        client.tesla.persist_configuration()
        return JsonResponse({'redirect_url': get_url_from_status(client)})

    return render(request, 'tesla_deploy_workers.html', context)
